...
const rules = {
    ...
    async (_parent, _args, context: Context) => {
      ...
      if (user.role !== "ADMIN") {
        return new AuthorizationError(
          "Administrator privileges required. Contact your system administrator for access."
        );
      }
  ...
  isManufactureOrAdmin: rule({ cache: "contextual" })(
    async (_parent, _args, context: Context) => {
      const user = await getUserWithRole(context);
      if (!user) {
        return new AuthenticationError(
      ...
      if (!["ADMIN", "MANUFACTURE"].includes(user.role)) {
        return new AuthorizationError(
          "This feature is only available to manufacturers and administrators."
        );
      }
  ...
  isCollectionOwner: rule({ cache: "strict" })(
    async (_parent, args, context: Context) => {
      ...
      if (userId !== collection.author?.id) {
        return new AuthorizationError(
          "You can only modify your own collections."
        );
      }
  ...
  isCollectionOwnerOrAdmin: rule({ cache: "strict" })(
    async (_parent, args, context: Context) => {
      const user = await getUserWithRole(context);
      if (!user) {
        return new AuthenticationError("Please login to access this resource.");
      ...
      if (user.id !== collection.author?.id) {
        return new AuthorizationError(
          "You can only access your own collections or be an admin."
        );
      }
