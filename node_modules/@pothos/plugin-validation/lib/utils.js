...
_export(exports, {
    get createArgsValidator () {
        return createArgsValidator;
    },
...
function createArgsValidator(argMappings, argsSchemas, options) {
    const argMapper = argMappings ? createInputValueMapper(argMappings, (value, mappings, addIssues)=>{
        ...
        return reduceMaybeAsync(mappings.value.typeSchemas, value, (val, schema)=>(0, _core.completeValue)(schema['~standard'].validate(val), (result)=>{
                if (result.issues) {
        ...
        return reduceMaybeAsync(mappings.value.fieldSchemas, mapped, (val, schema)=>(0, _core.completeValue)(schema['~standard'].validate(val), (result)=>{
                if (result.issues) {
                    addIssues(result.issues);
                    return null;
                }
    ...
    return function validateArgs(args, context, info) {
        return (0, _core.completeValue)(argMapper ? argMapper(args) : {
            value: args,
            issues: undefined
        }, (mapped)=>{
            if (mapped.issues) {
            ...
            const issues = [];
            const validated = reduceMaybeAsync(schemasArray, mapped.value, (val, schema)=>(0, _core.completeValue)(schema['~standard'].validate(val), (result)=>{
                    if (result.issues) {
            ...
            return (0, _core.completeValue)(validated, (result)=>{
                if (issues.length) {
...
function createInputValueMapper(argMap, mapType, mapField) {
    return function mapObject(obj, map = argMap, path = [], ...args) {
        ...
        map.forEach((field, fieldName)=>{
            ...
            if (field.kind === 'InputObject' && field.fields.map) {
                if (field.isList) {
                    mapped[fieldName] = mapListValue(fieldVal, field.listDepth, (val, i, newList, indices)=>{
                        ...
                        const promise = (0, _core.completeValue)(result, (newVal)=>{
                            if (newVal.issues) {
                        ...
                        if ((0, _core.isThenable)(promise)) {
                            fieldPromises.push(promise);
                        }
                        return promise;
                    });
                } else {
                    const promise = (0, _core.completeValue)(mapObject(fieldVal, field.fields.map, [
                        ...path,
                        fieldName
                    ], ...args), (newVal)=>{
                        if (newVal.issues) {
            ...
            const promise = (0, _core.completeValue)(fieldPromises.length ? Promise.all(fieldPromises) : null, ()=>{
                if (field.value !== null && !issues.length) {
                    if (field.isList) {
                        const list = mapListValue(mapped[fieldName], field.listDepth, (val, i, arr, indices)=>{
                            if (val != null) {
                                ...
                                if ((0, _core.isThenable)(result)) {
                                    promises.push((0, _core.completeValue)(result, (newVal)=>{
                                        arr[i] = newVal;
                                    }));
                                }
                                return result;
                        ...
                        return (0, _core.completeValue)(mapField(list, field, addIssues([
                            ...path,
                            fieldName
                        ]), ...args), (finalVal)=>{
                    ...
                    return (0, _core.completeValue)(mapType(mapped[fieldName], field, addIssues([
                        ...path,
                        fieldName
                    ]), ...args), (newVal)=>(0, _core.completeValue)(mapField(newVal, field, addIssues([
                            ...path,
                            fieldName
                        ]), ...args), (newVal)=>{
        ...
        return (0, _core.completeValue)(promises.length ? Promise.all(promises) : null, ()=>{
...
function reduceMaybeAsync(items, initialValue, fn) {
    function next(value, i) {
        ...
        return (0, _core.completeValue)(fn(value, items[i], i), (result)=>{
