...
const MANUFACTURER_TASKS_QUERY = `
      ...
      <TableCell className="text-sm max-w-xs">
        {task.collection ? (
          <span className="px-2 py-1 bg-purple-100 text-purple-800 rounded text-xs">
            {task.collection.modelCode} - {task.collection.name}
          </span>
        ) : (
          <span className="text-gray-400">-</span>
        )}
          ...
          <div className="flex items-center gap-2">
            <div className="w-16 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${task.productionTracking.progress}%` }}
              />
            </div>
            <span className="text-xs">{task.productionTracking.progress}%</span>
          </div>
        ) : (
          <span className="text-gray-400">-</span>
        )}
...
export default function ManufacturerTasksPage() {
  ...
  const [result] = useQuery({
    query: MANUFACTURER_TASKS_QUERY,
    variables: {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold">Üretim Görevlerim</h1>
          <p className="text-gray-600 mt-1">
            Müşteri siparişleri ve numune istekleri - Toplam {stats.total} görev
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-40">Koleksiyon</TableHead>
                <TableHead className="w-40">Üretim İlerleme</TableHead>
