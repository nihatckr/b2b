...
const PENDING_STAGE_APPROVALS_QUERY = gql`
...
export function PendingStageApprovals() {
  ...
  const [{ data, fetching }] = useQuery({
    query: PENDING_STAGE_APPROVALS_QUERY,
  ...
  // Filter trackings that need approval (6 hours before estimated end)
  const pendingApprovals = trackings.filter((tracking: any) => {
    // Must have an IN_PROGRESS status
    ...
    // Check if within 6 hours of completion
    const now = new Date();
  ...
  // Always show if there are any trackings (for testing)
  if (pendingApprovals.length === 0 && trackings.length === 0) {
    return null;
  }

  // If no pending approvals but have trackings, show empty state for testing
  if (pendingApprovals.length === 0 && trackings.length > 0) {
    return (
      <Card className="border-gray-200 bg-gray-50/50">
        ...
        <CardContent>
          <p className="text-sm text-gray-500">
            Şu anda onay bekleyen aşama bulunmuyor. Tüm üretim süreçleri
            zamanında ilerliyor.
          </p>
        </CardContent>
      </Card>
        ...
        <div className="space-y-3">
          {pendingApprovals.map((tracking: any) => {
            const currentStageUpdate = tracking.stageUpdates?.find(
              (u: any) => u.stage === tracking.currentStage
            );
