  ...
  const yoga = createYoga({
    ...
    plugins: [
      ...
      useJWT({
        ...
        signingKeyProviders: [
          createInlineSigningKeyProvider(process.env.JWT_SECRET || 'fallback-secret-only-for-dev')
        ],
      ...
      useReadinessCheck({
        ...
        check: async () => {
          try {
            ...
            if (isShuttingDown) {
              console.log('[Health Check] Server is shutting down, marking as not ready');
              return false;
            }
